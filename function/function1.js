// function add(a,b){
//     console.log(a*b)
// }
// add(30,322)//352
// add(1,2,3)//3
// add(1)//nan
// add(1,'rahi')//irahi
// add(22,33)
// // add(2,6)

//=========================================
// function ki parameter c=1 ichamu



// function add(a,b,c=1){
//     console.log(a+b+c)
// }
// add(3,3)//352
// add(1,2,3)//3
// add(1)//nan
// add(1,'rahi')//irahi
// add(22,33)
// add(2,6)



//==============================================


// function calage(year){

//     return(2025-year);
// }
//  let age1=calage(2005)
//  console.log(age1)


//======================================

// function cal_age(birth_year){
// let current_year=newdate().getfullyear()
// return current_year - birth_year;
// }
// let r=cal_age(1999)
// console.log(r);


//=============================================


//  let dt=new Date();

//  console.log("current date",dt.getDate())
//  console.log("current date",dt.getMonth())
//  console.log("current date",dt.getFullYear())


//===========================================
// function login(){

// console.log("gm")
// return("login success")
// }
// let msg=login();
// console.log(msg)

//==========================================


// function login1() {
//     console.log("Statement 1")
//     console.log("Statement 2")
//     console.log("Statement 3")
//     console.log("Statement 4")
//     console.log("Statement 5")

//     return "return statement"

//     console.log("Statement 6")
// }

// console.log("statement Before")

// let ret = login1()

// console.log(ret)
// console.log("statement after")

//===================================================

// let dt=new Date();
// console.log("current date:",dt.getDate())
// console.log("current month:",dt.getMonth())
// console.log("current year:",dt.getFullYear())

//==============================================



// function sum(num1,num2){
//     var result=num1+num2
//     return result
// }
// console.log(sum(2,2)) //function calling
//===========================================

//  var add=function sum(num1,num2){
//     var result=num1+num2
//     return result;5
// }
// var res= add(4,2)
//console.log( res);
//=================================


// //arrow function
// product=(num1,num2)=>{
//     var result=num1+num2
// console.log (result)
// }

// product(3,3)

//======================


// product=(num1,num2)=>num1*num2
// console.log(product(7,2))

//=====================

// product=num1=>num1*num1

// console.log(product(7))

//========================


// var greet=_=> console.log("good morning");
// greet()


//========================================

// var greet=_=>{
//      console.log("good morning")
//      console.log("k")
//      console.log("good")
//      return 3

    
// }
// var res=greet()
// console.log(res)

 //=======================================
//new functions


// {
//     console.log("gm")
//     console.log("gn")
//     console.log("ga")
// }



//  function wish () {
//     console.log("gm")
//     console.log("gn")
//     console.log("ga")
// }

// wish()




// function wish (a,b) {

// return a+b
// }

// let r1=wish(22,22)
// console.log(r1)


// let r2=wish(22,33)
// console.log(r2)






// function wish (a,b) {

//     console.log(a+b)
//     }
    
// wish(22,22)
    



// function add(a,b){
//     return a+b
//     console.log("goodmorning")//it is a un reachable statement
// }
// let r1=add(100,222)
// console.log


// function cal_age(year){

//     let current_year=2024
//     return current_year-year
// }

// var age=cal_age(2005)
// console.log(age)





// function cal_age(year){

// //10 overraide by year
//     return 2024-year,10 //it can't return multiple values
// }

// var age=cal_age(2005)
// console.log(age)




// function get_name(){


//         return "rajini","vijay","nay"
//     }
    
//     let name=get_name()
//     console.log(name)


// function get_name(){


//     return "rajini","vijay","nay"
// }

// console.log(get_name())




// function add (a,b){
//     console.log(a+b)
// }
// add(1,3)
// add(1,2,3)
// add(1,"rahun")
// add(1,)




// function add (a,b){
//     let c=1
//     return a+b+c
// }
// var r1=add(1,3)
// console.log(r1)
// let r2 =add(1,2,3)
// console.log(r2)
// add(1,"rahun")
// // add(1,)


// let dt=new Date()
// console.log("current date:",dt.getDate())
// console.log("current month:",dt.getMonth())
// console.log("current year:",dt.getFullYear()-2)






// function cal_age(birth_year){

//     return 2024 - birth_year;
// }
    
// let r_age=cal_age(2003)








// function login(a,b){
// return ("login success")
// // console.log("gn")
// }

// let msg=login(1,2)
// console.log(msg)




// let names =['rahul','sonia','priya','modi'];
// names.forEach(enames =>{
//     console.log(enames[0]);
// });

// let eids=[11,22,33,44,55,66,77,]

// console.log(eids.indexOf(99))



